...
from tidsense.receiver import TidsenseReceiver
...
if len(argv) < 2:
    print ("usage: %s tcp://sensor.server.hostname:1202 [node] [node]" %
        (argv[0]))
    exit(2)

server = argv[1]

r = TidsenseReceiver(server)

if len(argv) > 2:
    for arg in argv[2:]:
        r.subscribe(eval(arg))
else:
    r.subscribe_all()
...
while True:
    ...
    DOPPEL_SITE_URL = DOPPEL_BASE_URL + 'sites/4'
    ...
    if __name__ == '__main__':
        ...
        for device in [data]:
            ...
            for sensor in unitsDict:
                ...
                if sensor == "fault_charge":
                    value = device.get("charge_flags")["fault"]
                elif sensor == "charge":
